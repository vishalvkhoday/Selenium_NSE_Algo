...
def Tag_next_compare (BaseLine , C_Code,fname) :
    Base_split_tag = BaseLine.split('/')
    C_split_tag = C_Code.split('/')
    Base_tag_ctn = len (Base_split_tag)
    C_tag_ctn = len(C_split_tag)
    btag=0
    ctags=0
    MisTag_val_all=''
    MisTag_field_all=''
    
    #for btag in range (btag,Base_tag_ctn) :
    for btag in range (btag,Base_tag_ctn) :
        for ctags in range (ctags,C_tag_ctn):
            temp_b_tag = Split_tag(Base_split_tag[btag])
            t_spt_b_tag = temp_b_tag.split('-')
            temp_c_tag = Split_tag(C_split_tag[ctags])
            t_spt_c_tag = temp_c_tag.split('-')
            if len(t_spt_b_tag) !=0 and len(t_spt_b_tag)!=0 :
                if(t_spt_b_tag[0] ==t_spt_c_tag[0]):
                    print 'Actual : %s \t Expected : %s' %(temp_b_tag,temp_c_tag)
                    if temp_b_tag != temp_c_tag :
                        MisTag_val = ("Actual :  %s \t Expected : %s") % (temp_b_tag,temp_c_tag)
                        MisTag_val_all += MisTag_val + '\n'
                    btag =btag+1
                    ctags=ctags+1
#                     
                else :
                    print 'Actual : %s \t Expected : %s' %(temp_b_tag,temp_c_tag)
                    #Error_log(fname, 'Actual: ' +temp_b_tag +'\t Expected :'+ temp_c_tag)
                    Misfield_val = ("Actual :  %s \t Expected : %s") % (temp_b_tag,temp_c_tag)
                    MisTag_field_all += Misfield_val + '\n'
                    btag=btag+1
                    ctags=ctags-1
                    #break
                    continue
    ...
    Error_log(fname, MisTag_val_all +'\n'+MisTag_field_all)
...
def log_transaction (Fname, Outbound_msg):
    ...
    Log_file.write('\n***************************Log*************************** \n\n\n')
    str_len = len(Outbound_msg)
    str_st=0
    str_end =0
    
    while (str_st >str_len):
        if (str_len -str_st )>80:
            str_end= str_end+80
            print Outbound_msg[str_st:str_end]
            Log_file.write(Outbound_msg[str_st:str_end] + '\n')
            str_st = str_st+80
        else:
            print Outbound_msg[str_st:str_len] 
            Log_file.write(Outbound_msg[str_st:str_len] + '\n')
...
#             print FPath+"\\"+filename+str_Baselint_Input_str
            ...
            str_Base_OutBound = str_Baselint_Input_str[str_Base_OutBound_Ind +17 :len(str_Baselint_Input_str)]
...
# C Code comparsion starts from here 
    ...
    while 1:
        ...
        if len(C_line) == 0:
            ...
            str_C_code_OutBound = str_C_code_Input_str[str_C_code_OutBound_Ind+17:len(str_C_code_Input_str)]
            print str_C_code_OutBound
                        
            # print C_Path+"\\"+filename+str_C_code_Input_str
            fname = str(Results + "\\Failed_" + filename)
            
            #Tag_next_compare(str_Base_OutBound,str_C_code_OutBound,fname)
